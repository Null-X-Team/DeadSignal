...
DS.Engine = class Engine {
  ...
  buildDoors() {
    this.doors = [
      { id: 0, x: 280, kind: "shop", open: 0, target: 0, label: "ARMORY" },
    ];
    let id = 1;
  ...
  bind() {
    const down = (e) => {
      ...
      if (["Space", "ArrowLeft", "ArrowRight", "KeyB"].includes(e.code)) {
        e.preventDefault();
      }
    ...
    const point = (clientX, clientY) => {
      ...
      return {
        x: ((clientX - r.left) * DS.VIEW_W) / r.width,
        y: ((clientY - r.top) * DS.VIEW_H) / r.height,
      };
    };
  ...
  fire() {
    ...
    for (let i = 0; i < w.pellets; i++) {
      const a = aim + DS.rand(-w.spread, w.spread);
      this.bullets.push({
        x: muzzleX,
        y: muzzleY,
  ...
  pulseKick() {
    ...
    this.burst(this.player.x, this.player.y, "#ffbf70", 26, 240);
  ...
  update(dt) {
    ...
    this.doors.forEach((d) => {
      ...
      if (d.kind === "spawn" && d.open > 0.95 && this.phase === "combat") {
        const still = this.enemies.some(
          (e) => !e.dead && e.doorId === d.id && Math.abs(e.x - d.x) < 40,
        );
        if (!still) d.target = 0;
      }
    });
    ...
    for (let i = 0; i < pads.length; i++) {
      ...
      if (p.buttons[7] && p.buttons[7].pressed) this.fire();
    }

    this.player.vx = mx * this.player.speed;
    this.player.x = DS.clamp(
    ...
    this.enemies.forEach((e) => {
      ...
      e.y = DS.FLOOR_Y - (e.type === "brute" ? 52 : 42);
      e.facing = dx >= 0 ? 1 : -1;
